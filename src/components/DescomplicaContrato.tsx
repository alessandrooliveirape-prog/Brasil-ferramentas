 ...
 * 
 * Módulo DescomplicaContrato — Tool Brasil
 * Analisador de Contratos Gratuito com Diagnóstico Falado em até 2 Minutos
 * Client-Side PDF extraction via pdf.js CDN, Web Speech API nativa (pt-BR),
 * sanitização de dados pessoais no front-end e conteúdo educativo completo (Anti-Thin Content).
 */
...
export default function DescomplicaContrato() {
  ...
  // Configurar e selecionar voz em pt-BR da Web Speech API
  useEffect(() => {
    ...
    const loadVoices = () => {
      ...
      // Preferir voz em português brasileiro natural
      const ptBrVoice =
    ...
    const faqSchema = {
      ...
      mainEntity: [
        ...
        },
        {
          ...
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'Não assine o documento de imediato. Siga a recomendação falada indicada no veredito, solicitando formalmente por e-mail ou mensagem a alteração ou supressão da cláusula abusiva (especialmente multas desproporcionais ou renovação automática).'
          }
        }
      ]
    };
  ...
  const handlePdfFile = async (file: File) => {
    ...
    try {
      if (!window.pdfjsLib) {
        // Aguarda carregar biblioteca se ainda não estiver pronta
      ...
      // Verificação de PDF escaneado (sem texto)
      const cleanExtracted = extracted.replace(/--- PÁGINA \d+ ---/g, '').trim();
      if (cleanExtracted.length < 50) {
        setPdfWarning(
          '⚠️ Este arquivo PDF parece ser uma imagem digitalizada sem camada de texto (OCR). Por favor, copie e cole o texto do contrato na caixa abaixo ou utilize um arquivo digital com texto selecionável.'
        );
      } else {
        if (numPages > 10) {
          setPdfWarning(
  ...
  // Manipulação de arquivos arrastados ou selecionados
  ...
  const glossaryItems = [
    {
      ...
      explanation:
        'Cláusula que define qual cidade ou tribunal julgará brigas contratuais. Se a empresa colocar uma comarca muito distante da sua residência, ela pode inviabilizar sua defesa. Em relações de consumo (CDC), você tem o direito garantido de processar e ser processado no seu próprio domicílio.'
    },
    {
      term: 'Resilição Unilateral vs Rescisão',
      translation: 'Desistência voluntária vs Quebra por culpa de alguém',
      explanation:
        'Resilição unilateral é a desistência pura e simples do contrato antes do prazo (quando uma das partes apenas não quer mais continuar). Já a rescisão ocorre quando uma das partes desrespeita uma cláusula grave (inadimplência ou falta de entrega do serviço), justificando o cancelamento com penalidade.'
    },
    {
      term: 'Cláusula Penal',
      translation: 'Multa contratual por atraso ou cancelamento',
      explanation:
        'Nome jurídico para a multa aplicada caso você descumpra o contrato ou saia antes do prazo. No Código de Defesa do Consumidor, multas de atraso são limitadas a 2% e multas de cancelamento não podem ser desproporcionais, variando geralmente entre 10% e 20% do valor restante.'
    },
    {
      term: 'Caso Fortuito e Força Maior',
      translation: 'Acontecimentos imprevisíveis e fora do controle humano',
      explanation:
        'Situações extremas como enchentes, tempestades históricas, greves gerais ou pandemias. Quando ocorrem, liberam as partes de cumprir obrigações ou pagar multas durante o período de impossibilidade prática.'
    },
    {
      term: 'Adimplemento Substancial',
      translation: 'Quase tudo já foi pago ou cumprido',
      explanation:
        'Doutrina jurídica que impede o cancelamento abrupto do contrato ou a tomada de um bem quando a pessoa já cumpriu a imensa maioria da obrigação (por exemplo, já pagou 90% das parcelas). O credor pode cobrar o restante, mas não anular o negócio integralmente.'
    }
  ];
      ...
      <section className="bg-gradient-to-b from-white via-slate-50/50 to-white dark:from-slate-900 dark:via-slate-900/80 dark:to-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xs space-y-6">
        ...
        <div className="space-y-3 text-center max-w-3xl mx-auto">
          ...
          <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 font-medium leading-relaxed">
            ...
            Receba um diagnóstico falado e direto, sem juridiquês.
          </p>
        ...
        {errorMessage && (
          <div className="p-4 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-xl text-xs sm:text-sm text-red-800 dark:text-red-200 flex items-start gap-3">
          ...
          >
            ...
            <div className="bg-slate-900 text-white rounded-2xl p-5 sm:p-6 shadow-md space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1">
                ...
                {/* Indicador de reprodução ativa com animação de onda */}
                {isPlaying && (
            ...
            <div className="p-4 bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-600 dark:text-slate-300 flex items-start gap-2.5">
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {[
            ...
            },
            {
              ...
              a: 'Não assine o documento com pressa. Identifique a cláusula específica mencionada no veredito falado (como multas de rescisão desproporcionais ou renovação automática silenciosa) e solicite formalmente por escrito à empresa ou proprietário a modificação ou exclusão do trecho antes de fechar o acordo.'
            }
          ].map((faq, idx) => {
